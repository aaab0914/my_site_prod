...
def profile_edit(request):
    ...
    remaining = profile.get_avatar_change_remaining_days()
    return render(
        request, "users/profile_edit.html", {"form": form, "remaining_days": remaining}
    )
...
def profile(request, username=None):
    ...
    if viewing_own_profile:
        posts_queryset = Post.objects.filter(author=profile_user).order_by(
            "-publish", "-id"
        )
        comments_queryset = Comment.objects.filter(author=profile_user).order_by(
            "-created", "-id"
        )
        notes_queryset = Note.objects.filter(user=profile_user).order_by(
            "-updated", "-id"
        )
    else:
        posts_queryset = Post.published.filter(author=profile_user).order_by(
            "-publish", "-id"
        )
        comments_queryset = Comment.objects.filter(
            author=profile_user, active=True
        ).order_by("-created", "-id")
        notes_queryset = Note.objects.none()

    posts_page_obj = Paginator(posts_queryset, 10).get_page(
        request.GET.get("posts_page")
    )
    gallery_images = (
        ImagePost.objects.select_related("uploaded_by")
        .filter(uploaded_by=profile_user)
        .order_by("-created", "-id")[:10]
    )
    albums = (
    ...
    audio_posts = (
        ...
        .filter(uploaded_by=profile_user, active=True)
        .order_by("-created", "-id")[:10]
    )
    video_posts = (
    ...
    albums = [
        ...
        for album in albums
        if any(
            image.image and default_storage.exists(image.image.name)
            for image in album.images.all()
        )
    ]
    audio_posts = [
        item
        for item in audio_posts
        if item.audio_file and default_storage.exists(item.audio_file.name)
    ]
    video_posts = [
        item
        for item in video_posts
        if item.video_file and default_storage.exists(item.video_file.name)
    ]
...
def api_token_manage(request):
    ...
    return render(
        ...
        "users/api_token.html",
        {
            ...
            "token_regeneration_remaining_days": profile.get_token_regeneration_remaining_days()
            if profile
            else 0,
