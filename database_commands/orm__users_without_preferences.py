...
Run this on the server after SSH login:
    ...
    docker compose -f docker-compose.prod.yml exec -T web python database_commands/orm__users_without_preferences.py
"""
...
def main() -> None:
    ...
    rows = list(
        User.objects.filter(preferences__isnull=True)
        .order_by("id")
