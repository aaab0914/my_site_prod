...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        videos_without_cover = VideoPost.objects.filter(
            cover_image__isnull=True
        ) | VideoPost.objects.filter(cover_image="")
        ...
        for video in videos_without_cover:
            if video.video_file:
                self.stdout.write(
                    f"Processing: {video.title or video.get_video_filename()}"
                )
                ...
                if thumbnail:
                    ...
                    self.stdout.write(
                        self.style.ERROR("  ✗ Failed to extract thumbnail")
                    )

        self.stdout.write(
            self.style.SUCCESS(
                f"Completed: {success_count}/{count} thumbnails generated"
            )
        )
