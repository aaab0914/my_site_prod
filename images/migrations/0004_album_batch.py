...
def create_albums_for_existing_images(apps, schema_editor):
    ...
    for image in AlbumImage.objects.filter(album__isnull=True).order_by(
        "created", "id"
    ):
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.AddIndex(
            model_name="album",
            index=models.Index(
                fields=["created"], name="images_albu_created_8f0a4d_idx"
            ),
        ),
        migrations.RunPython(
            create_albums_for_existing_images, migrations.RunPython.noop
        ),
    ]
