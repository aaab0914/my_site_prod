...
class ProductionValidationScriptTests(SimpleTestCase):
    def setUp(self):
        ...
        self.validator = (
            self.validator_path.read_text(encoding="utf-8")
            if self.validator_path.exists()
            else ""
        )
        self.restore_script_path = BASE_DIR / "restore_test.ps1"
        self.restore_script = (
            self.restore_script_path.read_text(encoding="utf-8")
    ...
    def test_prod_env_validator_exists_and_checks_core_settings(self):
        ...
        self.assertIn(
            "CSRF_TRUSTED_ORIGINS must not be empty in production.", self.validator
        )
        self.assertIn(
            "SECURE_HSTS_SECONDS must be at least 31536000 in production.",
            self.validator,
        )
        self.assertIn(
            "DB_PASSWORD is too weak or still uses a placeholder value.", self.validator
        )
