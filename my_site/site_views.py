...
def render_public_cached_template(
    request, cache_key, template_name, context=None, timeout=SITE_HTML_CACHE_TTL
):
    context = context or {}
    ...
    if cached_payload is not None:
        response = HttpResponse(
            cached_payload["content"], content_type=cached_payload["content_type"]
        )
...
def site_index(request):
    return render_public_cached_template(
        request, "view:site_index", "index.html", timeout=300
    )
